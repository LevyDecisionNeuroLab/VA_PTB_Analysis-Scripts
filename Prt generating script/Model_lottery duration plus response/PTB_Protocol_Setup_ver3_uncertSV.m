...
par = readtable('E:\Ruonan\Projects in the lab\VA_RA_PTB\Clinical and behavioral\par nonpar att_allSubj_day1day2_08262019.csv');
...
ParametricModType = {'uncertSV', 'uncertSV_Sal'};
...
PRT.Color_Loss =      '0 0 255';
PRT.Color_Gains =     '255 0 0';
...
for i = 1:length(SubjectNums)
    for j = 1:length(gainsloss)
        for k = 1:length(ParametricModType)
            % for parametric design matrices, 4 conditions (2 lottery
            % duration + 1 responses)
            PRT.NrOfConditions =      '3';
            PTB_Protocol_Gen_ver3_uncertSV(SubjectNums(i), gainsloss{j}, ...
