...
for i = 1:length(SubjectNums)
    for j = 1:length(gainsloss)
        for k = 1:length(ParametricModType)
            % for non parametric design matrices, only 3 condistions (2 lottery duration + 1 responses)
            % for parametric design matrices, 4 conditions (2 lottery
            % duration + 1 empty parametic + 1 responses)
            if strcmp(ParametricModType{k}, 'none')
                PRT.NrOfConditions =      '3';
            else
                PRT.NrOfConditions =      '4';
            end
            PTB_Protocol_Gen_ver3_singleSV_subj95(SubjectNums(i), gainsloss{j}, ...
