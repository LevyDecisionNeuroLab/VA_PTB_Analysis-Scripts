...
path_out = 'E:\Ruonan\Projects in the lab\VA_RA_PTB\Analysis Ruonan\Fitpar files\Behavior data fitpar_101920';
...
%     subject_idx = 2;
    ...
    % columns: isDay1, isGain, isAmbig, sv, label
    % 124 is the number of trials per domain
    sv_table = zeros(2*124,5);
...
%         domain_idx = 0;
       ...
       %% model-fitting parameters
        ...
        sv_table((1+124*(domain_idx-1)):(124*domain_idx),1) = trial_isDay1;
        sv_table((1+124*(domain_idx-1)):(124*domain_idx),2) = repmat(is_gains,124,1);
        sv_table((1+124*(domain_idx-1)):(124*domain_idx),3) = trial_isAmbig;
        sv_table((1+124*(domain_idx-1)):(124*domain_idx),4) = sv;
    end
    
    %% bin trials based on SV, separate risk and ambig
    bin_n = 5; % number of bins
    
    % mask of risk and ambiguous trials
    mask_ambig = sv_table(:,3) == 1;
    mask_risk = sv_table(:,3) == 0;
    
    % find critical values and lable trials 1,2,3,4,5
    sv_ambig = sv_table(mask_ambig, 4);
    sv_ambig_uniq = unique(sv_ambig); 
    critical_ambig = quantile(sv_ambig_uniq, [.2, .4, .6, .8]);   
    ...
    label_ambig(sv_ambig <= critical_ambig(3) & sv_ambig > critical_ambig(2)) = 3;
    label_ambig(sv_ambig <= critical_ambig(4) & sv_ambig > critical_ambig(3)) = 4;
    label_ambig(sv_ambig > critical_ambig(4)) = 5;
    
    sv_risk = sv_table(mask_risk, 4);
    sv_risk_uniq = unique(sv_risk); 
    critical_risk = quantile(sv_risk_uniq, [.2, .4, .6, .8]);   
    ...
    label_risk(sv_risk <= critical_risk(3) & sv_risk > critical_risk(2)) = 3;
    label_risk(sv_risk <= critical_risk(4) & sv_risk > critical_risk(3)) = 4;
    label_risk(sv_risk > critical_risk(4)) = 5;
    
    sv_table(mask_ambig,5) = label_ambig;
    sv_table(mask_risk,5) = label_risk;
    
   %% save
   
   % gains
   gdata.sv_label = sv_table(sv_table(:,2)==1,5);
   ldata.sv_label = sv_table(sv_table(:,2)==0,5);
