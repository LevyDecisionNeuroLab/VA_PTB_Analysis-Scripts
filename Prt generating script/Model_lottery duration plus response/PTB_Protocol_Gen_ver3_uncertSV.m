function PTB_Protocol_Gen_ver3_uncertSV(subjectNum, gainsloss, tr, trialduration, DiscAcq, ParametricMod, path_in, path_out, PRT, par)
...
permissible_parameters = {'uncertSV', 'uncertSV_Sal'};
...
if ~is_gains
  Colors = {PRT.Color_Loss};
  alt_colors = {PRT.Color_Gains};
else
  Colors = {PRT.Color_Gains};
  alt_colors = {PRT.Color_Loss};
end
...
for blocknum = 1:4
  if subjectNum == 95 && (blocknum == 3 || blocknum ==4) && ~is_gains  
     continue % skip the for loop because sub 95 did not have these blocks
  end        
...
%   end
   ...
   if strcmp(ParametricMod, 'uncertSV_Sal')
       if is_gains
          block_amb = [block_amb block_sv(amb_index)];
          block_risk = [block_risk block_sv(risk_index)];
       elseif ~is_gains
   ...
   elseif strcmp(ParametricMod, 'uncertSV')
      block_amb = [block_amb block_sv(amb_index)];
      block_risk = [block_risk block_sv(risk_index)];     
   end
