...
function [info,p] = fit_risk_model_Constrained(choice,vF,vA,pF,pA,AL,model,b0,base)
% If multiple model fits requested, loop and pack

if iscell(model)
   for i = 1:length(model)
      [info(i),p{i}] = fit_risk_model_Constrained(choice,vF,vA,pF,pA,AL,model{i});
   end
   return;
end
...
%     [b,negLL,exitflag,convg] = fmincon(@local_negLL,b00,[],[],[],[],[-inf -3.67 .0894],[inf 4 4.34],[],OPTIONS,choice,vF,vA,pF,pA,AL,model,base);
    [b,negLL,exitflag,convg] = fmincon(@local_negLL,b00,[],[],[],[],[-inf 0.0905],[inf 7.6036],[],OPTIONS,choice,vF,vA,pF,pA,AL,model,base);
