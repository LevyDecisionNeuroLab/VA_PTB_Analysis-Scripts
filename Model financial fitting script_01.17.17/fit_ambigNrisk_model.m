...
function [info,p] = fit_ambigNrisk_model(choice,vF,vA,pF,pA,AL,model,b0);
...
if exist('fminunc','file')
   try
      ...
      OPTIONS = optimset('Display','off','LargeScale','off');
          [b,negLL,exitflag,convg,g,H] = fminunc(@local_negLL,b0,OPTIONS,choice,vF,vA,pF,pA,AL,model);
...
if strcmp(optimizer,'fminsearch')
   ...
   [b,negLL,exitflag,convg] = fminsearch(@local_negLL,b0,OPTIONS,choice,vF,vA,pF,pA,AL,model);
end
...
p = choice_prob_ambigNrisk(vF,vA,pF,pA,AL,b,model);
...
%----- LOCAL FUNCTIONS
function sumerr = local_negLL(beta,choice,vF,vA,pF,pA,AL,model);
